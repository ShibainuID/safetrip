...
export default function TrackPage() {
  return (
    <div className="flex flex-col gap-5">
      {/* Origin / destination */}
      <section className="flex flex-col gap-4 rounded-2xl border border-hairline bg-white p-5">
      ...
      <section>
        ...
        <div className="flex items-center justify-between rounded-2xl border border-hairline bg-surface-strong px-5 py-3.5">
          {COMMUTE_MODES.map((mode) => (
            <button
              key={mode.id}
              title={mode.id}
              className="flex h-12 w-12 items-center justify-center overflow-hidden rounded-xl border border-hairline bg-white p-1.5 transition-transform hover:scale-105 active:scale-95"
            >
              <Image src={mode.img} alt={mode.id} width={48} height={48} className="h-full w-full object-contain" />
            </button>
          ))}
        </div>
      </section>
      ...
      {/* Map */}
      <FakeMap route className="h-72 w-full overflow-hidden rounded-2xl border border-hairline" />
